...
describe('动物诊疗管理闭环', () => {
  ...
  it('完成诊疗机构备案到年度报告提交的完整数据流', async () => {
    ...
    const drug = await mockApi.stockInDrug({ institutionId: clinic.id, drugName: '阿莫西林克拉维酸钾片', batchNo: 'DRUG202606', manufacturer: '安徽动物药业有限公司', approvalNo: '兽药字120102001', validTo: '2028-06-30', quantity: 80, supplier: '省级兽药配送中心', traceCode: 'TRACE-AH-DRUG-001' })
    ...
    expect(submitted.petCount).toBe(1)
    expect(submitted.immunizationCount).toBe(1)
    expect(submitted.prescriptionCount).toBe(1)
    expect(submitted.drugStockInQuantity).toBe(80)
    expect(submitted.drugStockOutQuantity).toBe(6)
    expect(submitted.wasteHandledCount).toBe(1)
  ...
  it('阻断未备案通过兽医开具处方', async () => {
    ...
    const pet = await mockApi.createPetProfile({ ownerId: owner.id, name: '雪球', species: '猫', breed: '英短', gender: '雌性', age: 1, identityNo: 'IMM-AH-0002' })
    const drug = await mockApi.stockInDrug({ institutionId: clinic.id, drugName: '宠物益生菌', batchNo: 'DRUG202607', manufacturer: '安徽动物药业有限公司', approvalNo: '兽药字120102002', validTo: '2028-07-30', quantity: 20, supplier: '省级兽药配送中心', traceCode: 'TRACE-AH-DRUG-002' })
  ...
  it('阻断库存不足处方并保留库存', async () => {
    ...
    const pet = await mockApi.createPetProfile({ ownerId: owner.id, name: '可乐', species: '犬', breed: '金毛', gender: '雄性', age: 3, identityNo: 'IMM-AH-0003' })
    const drug = await mockApi.stockInDrug({ institutionId: clinic.id, drugName: '犬用驱虫片', batchNo: 'DRUG202608', manufacturer: '安徽动物药业有限公司', approvalNo: '兽药字120102003', validTo: '2028-08-30', quantity: 1, supplier: '省级兽药配送中心', traceCode: 'TRACE-AH-DRUG-003' })
